...
module PG
  class BasicTypeMapForQueries < PG::TypeMapByClass
    ...
    def init_encoders
      coders.each { |kl, _c| self[kl] = nil } # Clear type map
    ...
    def array_encoders_by_klass
      DEFAULT_ARRAY_TYPE_MAP.each_with_object({}) do |(klass, (format, name)), h|
    ...
    def get_array_type(value)
      elem = value
      elem = elem.first while elem.is_a?(Array)
