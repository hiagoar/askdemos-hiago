...
module PG
  class Connection
    ...
    POSTGRESQL_URI = %r{\A(?<URI>(?<scheme>[A-Za-z][+\-.0-9A-Za-z]*):(?<hier-part>//(?<authority>(?:(?<userinfo>(?:%\h\h|[!$&-.0-;=A-Z_a-z~])*)@)?(?<hostports>#{HOST_AND_PORT}(?:,\g<hostport>)*))(?<path-abempty>(?:/(?<segment>(?:%\h\h|[!$&-.0-;=@-Z_a-z~])*))*)|(?<path-absolute>/(?:(?<segment-nz>(?:%\h\h|[!$&-.0-;=@-Z_a-z~])+)(?:/\g<segment>)*)?)|(?<path-rootless>\g<segment-nz>(?:/\g<segment>)*)|(?<path-empty>))(?:\?(?<query>[^#]*))?(?:\#(?<fragment>(?:%\h\h|[!$&-.0-;=@-Z_a-z~/?])*))?)\z}
    ...
    def self.parse_connect_args(*args)
      ...
      if args.length == 1
        ...
        when URI, POSTGRESQL_URI
          ...
          iopts = URI.decode_www_form(uri_match['query']).to_h.transform_keys(&:to_sym) if uri_match['query']
          ...
          iopts[:host] = uri_match['hostports'].split(',', -1).map do |hostport|
            ...
            hostmatch['IPv6address'] || hostmatch['IPv4address'] || hostmatch['reg-name']&.gsub(/%(\h\h)/) do
        ...
        if args.length > max
          raise ArgumentError,
                format('Extra positional parameter %d: %p', max + 1, args[max])
        end
      ...
      if (host = iopts[:host]) && !iopts[:hostaddr]
        hostaddrs = host.split(',', -1).map do |mhost|
          if !mhost.empty? && !mhost.start_with?('/') && # isn't UnixSocket
             ...
             (RUBY_PLATFORM !~ /mingw|mswin/ || mhost !~ %r{\A\w:[/\\]})
      ...
      unless iopts[:fallback_application_name]
        oopts[:fallback_application_name] = $PROGRAM_NAME.sub(/^(.{30}).{4,}(.{30})$/) do
      ...
      if uri
        ...
        option_string + connect_hash_to_string(oopts)
      end
    end
    ...
    def copy_data(sql, coder = nil)
      ...
      when PGRES_COPY_IN
        begin
          if coder
            old_coder = encoder_for_put_copy_data
            self.encoder_for_put_copy_data = coder
          end
          yield res
        rescue Exception => e
          errmsg = format('%s while copy data: %s', e.class.name, e.message)
          put_copy_end(errmsg)
          get_result
          raise
        else
      ...
      when PGRES_COPY_OUT
        begin
          if coder
            old_coder = decoder_for_get_copy_data
    ...
    def self.conndefaults_hash
      conndefaults.each_with_object({}) do |info, hash|
    ...
    ### See also #conninfo
    def conninfo_hash
      conninfo.each_with_object({}) do |info, hash|
        hash[info[:keyword].to_sym] = info[:val]
      end
    end

    # Method 'ssl_attribute' was introduced in PostgreSQL 9.5.
    if instance_methods.find { |m| m.to_sym == :ssl_attribute }
      # call-seq:
    ...
    def get_copy_data(async = false, decoder = nil)
      if async
        sync_get_copy_data(async, decoder)
      else
    ...
    def put_copy_data(buffer, encoder = nil)
      flush until sync_put_copy_data(buffer, encoder)
      flush
    end
    ...
    def put_copy_end(*args)
      flush until sync_put_copy_end(*args)
      flush
    end
    ...
    def cancel
      ...
      if Fiber.respond_to?(:scheduler) && Fiber.scheduler && RUBY_PLATFORM =~ /mingw|mswin/
        ...
        cl = Thread.new(socket_io.remote_address, &:connect).value
        begin
        ...
        begin
          ...
          cl = if adr.ip?
                 TCPSocket.new(adr.ip_address, adr.ip_port)
               else
                 UNIXSocket.new(adr.unix_path)
               end
          cl.autoclose = false
        end
    ...
    private def async_connect_or_reset(poll_meth)
      ...
      until [PG::PGRES_POLLING_OK, PG::PGRES_POLLING_FAILED].include?(poll_status)
    ...
    class << self
      ...
      def new(*args, **kwargs)
        conn = connect_start(*args, **kwargs) or
      ...
      REDIRECT_CLASS_METHODS = {
        new: %i[async_connect sync_connect],
        connect: %i[async_connect sync_connect],
        open: %i[async_connect sync_connect],
        setdb: %i[async_connect sync_connect],
        setdblogin: %i[async_connect sync_connect],
        ping: %i[async_ping sync_ping]
      }.freeze

      # These methods are affected by PQsetnonblocking
      REDIRECT_SEND_METHODS = {
        isnonblocking: %i[async_isnonblocking sync_isnonblocking],
        nonblocking?: %i[async_isnonblocking sync_isnonblocking],
        put_copy_data: %i[async_put_copy_data sync_put_copy_data],
        put_copy_end: %i[async_put_copy_end sync_put_copy_end],
        flush: %i[async_flush sync_flush]
      }.freeze
      REDIRECT_METHODS = {
        :exec => %i[async_exec sync_exec],
        :query => %i[async_exec sync_exec],
        :exec_params => %i[async_exec_params sync_exec_params],
        :prepare => %i[async_prepare sync_prepare],
        :exec_prepared => %i[async_exec_prepared sync_exec_prepared],
        :describe_portal => %i[async_describe_portal sync_describe_portal],
        :describe_prepared => %i[async_describe_prepared sync_describe_prepared],
        :setnonblocking => %i[async_setnonblocking sync_setnonblocking],
        :get_result => %i[async_get_result sync_get_result],
        :get_last_result => %i[async_get_last_result sync_get_last_result],
        :get_copy_data => %i[async_get_copy_data sync_get_copy_data],
        :reset => %i[async_reset sync_reset],
        :set_client_encoding => %i[async_set_client_encoding sync_set_client_encoding],
        :client_encoding= => %i[async_set_client_encoding sync_set_client_encoding],
        :cancel => %i[async_cancel sync_cancel]
      }.freeze

      if PG::Connection.instance_methods.include? :async_encrypt_password
        REDIRECT_METHODS.merge!({
                                  encrypt_password: %i[async_encrypt_password sync_encrypt_password]
                                })
      end
