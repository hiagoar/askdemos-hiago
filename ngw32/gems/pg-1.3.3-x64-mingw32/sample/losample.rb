...
SAMPLE_WRITE_DATA = 'some sample data'.freeze
SAMPLE_EXPORT_NAME = 'lowrite.txt'.freeze
...
puts 'Import test:'
puts format('  importing %s', __FILE__)
oid = conn.lo_import(__FILE__)
puts format('  imported as large object %d', oid)
...
puts format('  appended %d bytes', totalbytes)
...
puts format('  exported as %s (%d bytes)', SAMPLE_EXPORT_NAME, File.size(SAMPLE_EXPORT_NAME))
