...
conn.transaction do
  ...
  begin
    while copy_data.read(256, buf)
      ...
      warn format("\tsending %d bytes of data...", buf.length)
    ...
    errmsg = format('%s while reading copy data: %s', e.class.name, e.message)
    conn.put_copy_end(errmsg)
  else
    conn.put_copy_end
    while res = conn.get_result
      warn format('Result of COPY is: %s', res.res_status(res.result_status))
    end
  end
end
