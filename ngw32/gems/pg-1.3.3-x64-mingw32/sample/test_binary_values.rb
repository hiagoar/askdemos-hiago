...
db.exec('SELECT a::int, b::bytea FROM test LIMIT 1', [], 1) do |res|
  res.nfields.times do |i|
    puts format('Field %d is: %s, a %s (%s) column from table %p', i, res.fname(i),
                db.exec('SELECT format_type($1,$2)', [res.ftype(i), res.fmod(1)]).getvalue(0, 0), res.fformat(i).zero? ? 'string' : 'binary', res.ftable(i))
  end
