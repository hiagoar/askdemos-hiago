...
File.open(ec_def, 'w') do |fd_def|
  ...
  File.read(ec_txt).lines.each do |line|
    ...
    next unless line =~ /^(\w+)\s+(\w+)\s+(\w+)\s+(\w+)\s+/i

    sqlstate = Regexp.last_match(1)
    ews = Regexp.last_match(2)
    errcode_macro_name = Regexp.last_match(3)
    next unless ews == 'E'

    is_sqlclass = sqlstate[2..] == '000'
    ...
    fd_def.puts "  register_error_class( #{class_code.inspect}, klass );" if is_sqlclass
    fd_def.puts '}'
  end
end
