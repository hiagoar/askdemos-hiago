...
if enable_config('gvl-unlock', true)
  ...
  warn 'Calling libpq with GVL unlocked'
else
  warn 'Calling libpq with GVL locked'
end
...
if enable_config('windows-cross')
  ...
  if pgconfig && pgconfig != 'ignore'
    warn format('Using config values from %s', pgconfig)
    ...
    dlldir = RUBY_PLATFORM =~ /mingw|mswin/ ? IO.popen([pgconfig, '--bindir'], &:read).chomp : libdir
          ...
          dlldir = ldflags && ldflags[/-L([^ ]+)/] && Regexp.last_match(1)
        end
  ...
  # on MacOS-X.
  append_ldflags "-Wl,-rpath,#{dlldir.quote}" if dlldir && RbConfig::CONFIG['RPATHFLAG'].to_s.empty?
end

warn "Using libpq from #{dlldir}"
...
append_cppflags('-D__EXTENSIONS__') if RUBY_VERSION >= '2.3.0' && /solaris/ =~ RUBY_PLATFORM
